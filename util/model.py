...
def generate_actions(self, tokens, max_action_len=32, strategy='greedy', temperature=1.0):
    ...
    action_masks_pi = tokens['action_masks'] if 'action_masks' in tokens else None
...
class Policy(nn.Module):
    ...
    @torch.no_grad()
    def generate_action_high(self, state_ids):
        ...
        outputs = self.base.generate(
            **state_ids,
            ...
            do_sample=False,
            temperature=self.args.get("temperature", 0.0),
            top_p=0.9, 
            top_k=50,  
        )
...
class HighPolicy(Policy):
    def __init__(self, args):
        super().__init__(args)
        if args.get("use_lora_high", False):
            self.base = _apply_lora(
                self.base,
...
class LowPolicy(Policy):
    def __init__(self, args):
        super().__init__(args)
        if args.get("use_lora_low", False):
            self.base = _apply_lora(
                self.base,
